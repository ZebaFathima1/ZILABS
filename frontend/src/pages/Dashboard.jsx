...
import { STUDENT, PROJECTS, BADGES, TRACKS } from '../mock/mockData';
    ...
    <main className="relative pt-28 pb-20 min-h-screen overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-6">
        ...
        <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <div className="relative">
              <img src={STUDENT.avatar} alt="" className="w-16 h-16 rounded-2xl ring-2 ring-cyan-400/50" />
              <div className="absolute -bottom-1 -right-1 px-1.5 py-0.5 rounded-md bg-gradient-to-r from-cyan-400 to-purple-500 text-[10px] font-bold text-white">LV {STUDENT.level}</div>
            </div>
            <div>
              ...
              <div className="text-xs text-white/55">{STUDENT.track} · ID {STUDENT.id}</div>
            </div>
          </div>
        ...
        <div className="mt-8 grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard icon={Zap} label="Total XP" value={STUDENT.xp.toLocaleString()} sub={`Level ${STUDENT.level}`} color="#00E5FF" />
          <StatCard icon={Flame} label="Streak" value={`${STUDENT.streakDays} days`} sub="Don’t break it" color="#F472B6" />
          <StatCard icon={Calendar} label="Attendance" value={`${STUDENT.attendance}%`} sub="Last 30 days" color="#00FFA3" />
          <StatCard icon={Trophy} label="Rank" value="#1" sub="AI & ML track" color="#FFD700" />
        </div>

        <div className="mt-6 grid lg:grid-cols-3 gap-6">
          {/* XP progress + projects */}
            ...
            <div className="zv-card p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="mt-1 font-display text-xl font-semibold">{STUDENT.xp.toLocaleString()} / {STUDENT.xpNext.toLocaleString()} XP</div>
                </div>
            ...
            <div className="zv-card p-6">
              ...
              <div className="mt-4 space-y-3">
                {STUDENT.assignedProjects.map(ap => {
                  const p = PROJECTS.find(x => x.id === ap.id);
                  return (
                    <div key={ap.id} className="flex items-center gap-4 p-3 rounded-xl bg-white/[0.03] border border-white/5 hover:border-cyan-400/30 transition">
                      ...
                      <button className="text-cyan-300 hover:text-cyan-200 text-sm">Open</button>
                    </div>
                  );
                })}
              </div>
          ...
          {/* Side: Badges + achievements + share */}
          <div className="space-y-6">
            <div className="zv-card p-6">
              ...
              <div className="mt-4 flex items-center gap-2">
                <button className="flex-1 zv-btn-ghost !py-2 text-xs inline-flex items-center justify-center gap-1.5"><Linkedin size={12} /> Share</button>
                <button className="flex-1 zv-btn-ghost !py-2 text-xs inline-flex items-center justify-center gap-1.5"><Share2 size={12} /> Copy link</button>
              </div>
            </div>
