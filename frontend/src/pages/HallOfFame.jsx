...
import { getLeaderboard, getApiErrorMessage } from '../services/api';
  ...
  return (
    <main className="relative pt-28 pb-20 min-h-screen overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-6">
        ...
        {leadersQuery.isLoading && <LoadingBlock label="Loading leaderboard…" />}

        {leadersQuery.isError && (
          <ErrorBlock
            message={getApiErrorMessage(leadersQuery.error, 'Failed to load leaderboard.')}
            onRetry={() => leadersQuery.refetch()}
          />
        )}

        {!leadersQuery.isLoading && !leadersQuery.isError && leaders.length === 0 && (
          <div className="mt-16 text-center text-white/50">No leaderboard data available yet.</div>
        )}

        {!leadersQuery.isLoading && !leadersQuery.isError && top3.length >= 3 && (
        ...
        {!leadersQuery.isLoading && !leadersQuery.isError && rest.length > 0 && (
        ...
        {!leadersQuery.isLoading && !leadersQuery.isError && leaders.length > 0 && (
          <div className="mt-6 grid md:grid-cols-3 gap-4">
            ...
            <Highlight icon={Trophy} title="Capstone of the month" value="Fraud Detection" sub={`by ${leaders[0]?.name || 'a builder'}`} />
          </div>
        )}
      </div>
    </main>
