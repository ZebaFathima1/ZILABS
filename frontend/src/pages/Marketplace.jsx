...
import { PROJECTS, TRACKS } from '../mock/mockData';

const icons = { aiml: Brain, fullstack: Layers, data: Database, cyber: Lock, cloud: Cloud, genai: Sparkles };

// Motion variants extracted to prevent inline object recreation on every render
...
const Marketplace = () => {
  ...
  // PROJECTS is a module-level constant import, not a reactive dependency
  ), [q, track, diff]);
    ...
    <main className="relative pt-28 pb-20 min-h-screen overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-6">
        <motion.div {...HEADER_MOTION} className="max-w-3xl">
          <div className="text-[11px] uppercase tracking-[0.3em] text-cyan-300/80 font-mono">Project Marketplace</div>
          ...
          <p className="text-white/60 mt-3">Industry-aligned projects with weekly mentor reviews, rubric-based grading, and verifiable credentials.</p>
        </motion.div>
        ...
        <div className="mt-8 zv-glass rounded-2xl p-3 flex flex-col md:flex-row gap-3 items-stretch md:items-center">
          ...
          <div className="flex gap-2">
            ...
            <button className="inline-flex items-center gap-2 px-3 py-2 rounded-xl bg-white/[0.04] border border-white/10 text-sm hover:border-cyan-400/50"><SlidersHorizontal size={14} /> Filters</button>
          </div>
        </div>
                ...
                <TiltCard className="zv-card overflow-hidden h-full group">
                  ...
                  <div className="p-5">
                    ...
                    <div className="mt-4 flex items-center justify-between">
                      ...
                      <Link to="/login" className="inline-flex items-center gap-1.5 text-sm text-cyan-300 hover:text-cyan-200">Start <ArrowUpRight size={14} /></Link>
                    </div>
                  </div>
                </TiltCard>
              </motion.div>
...
const Chip = ({ active, onClick, label, color }) => (
  <button onClick={onClick} className={`px-3 py-1.5 rounded-full text-xs border transition ${active ? 'border-transparent' : 'border-white/10 hover:border-white/25'}`}
